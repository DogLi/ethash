...
mod test {
    ...
    fn proofs() {
        ...
        assert_eq!(
            ...
            hex::decode("d9a38e294d953b1e735e8e71025a1855ed7f2139e13ff8a19bb7e82383576c47")
                .unwrap()
        );

        let dag = ethash::LightDAG::new(header.number, false);
        let (mix_hash, result) = dag.compute_light(header_hash, header.nonce);
        assert_eq!(
            result.as_bytes(),
            hex::decode("000000003a0a4fb7f886bad18226a47fb09767ac8c0c87141083443ac5cfdf59")
                .unwrap()
        );
        ...
        assert_eq!(
            ...
            &[
                ...
                5274872, 3725170, 202134, 5492399, 6895738, 5696426, 6626457, 2345861, 262304,
                2658959, 7286807, 547777, 5472769, 7664032, 1035384, 2671289, 4103686, 8347077,
                2322872, 6754122, 2654051, 4610695, 65291, 3601125, 1821797, 5122957, 5336515,
                7610054, 652865, 375080, 5367006, 2543741, 2475727, 341558, 5858560, 7361407,
                3569253
            ]
        );
        ...
        let (depth, leaves) = ethash::calc_dataset_merkle_leaves(dag.epoch, &dataset);
        ...
        for index in &indices {
            ...
            let (element, _leaf_hash, proofs) = tree.generate_proof(*index as _, depth);
    ...
    fn mix_hash_2() {
        ...
        dbg!(&header);
        let dag = ethash::LightDAG::new(header.number, false);
        let (mix_hash, _) = dag.compute_light(header.seal_hash(), header.nonce);
        assert_eq!(mix_hash, header.mix_hash);
    }
